...
<!--
 ### SQL Code Block Switcher

  We have an extended syntax type for different SQL flavors that allows you to provide an example for each, and the display will render the code block with a dropdown to switch between them. [Examples on the hub](https://hub.steampipe.io/plugins/turbot/aws/tables/aws_account#basic-aws-account-info) use this capability.
    - ***Does this work everywhere?  or only on the hub?***
    - ***Do we have other custom types for OS'es or anything?***
    - supported tags are  `sql+postgres`, `sql+sqlite` (***any others?? `sql+mysql`?? `sql+duckdb`??***)
    - The examples must appear one after another with only a link break between them:
